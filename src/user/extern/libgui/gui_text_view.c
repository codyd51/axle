...
void gui_text_view_add_subview(gui_view_t* superview, gui_text_view_t* subview) {
	return gui_scroll_view_add_subview(superview, (gui_scroll_view_t*)subview);
}

void gui_text_view_add_to_window(gui_text_view_t* view, gui_window_t* window) {
	return gui_scroll_view_add_to_window((gui_scroll_view_t*)view, window);
}
...
static void _gui_text_view_newline(gui_text_view_t* text_view) {
	text_view->cursor_pos.x = 0;
	text_view->cursor_pos.y += text_view->font_size.height + 6;

	if (text_view->cursor_pos.y + text_view->font_size.height >= text_view->content_layer->scroll_layer.inner->size.height) {
		...
		text_view->cursor_pos = point_zero();
		text_view->content_layer->scroll_layer.max_y = 0;
	}
}
