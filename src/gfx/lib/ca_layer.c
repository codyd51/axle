...
void blit_layer_filled(ca_layer* dest, ca_layer* src, Rect src_frame) {
	//copy row by row
	
	Coordinate origin = src_frame.origin;
	
	//offset into dest that we start writing
	uint8_t* dest_row_start = dest->raw + (rect_min_y(src_frame) * dest->size.width * gfx_bpp()) + (rect_min_x(src_frame) * gfx_bpp());

	//data from source to write to dest
	//offset by x origin
	//uint8_t* row_start = src->raw + (origin.x * gfx_bpp());
	uint8_t* row_start = src->raw;
	//add row length origin.y times
	//row_start += (src->size.width * origin.y * gfx_bpp());

	//copy height - y origin rows
	//for (int i = 0; i < src_frame.size.height - origin.y; i++) {
	for (int i = 0; i < src_frame.size.height; i++) {
	//	memcpy(dest_row_start, row_start, (src_frame.size.width - origin.x) * gfx_bpp());
...
void blit_layer(ca_layer* dest, ca_layer* src, Rect src_frame) {
	...
	if (src->alpha >= 1.0) {
		...
		blit_layer_filled(dest, src, src_frame);
	}
	else if (src->alpha <= 0) {
		//do nothing
		return;
		...
		blit_layer_alpha(dest, src, src_frame);
	}
}
